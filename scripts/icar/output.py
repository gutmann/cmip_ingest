...
def write_file(date,info,cmip):
    ...
    atts=Bunch(standard_name="eastward_wind", long_name="U (E/W) wind speed",units="m s**-1")
    extra_vars.append(Bunch(name="u",data=cmip["u"],dims=dims,dtype="f",attributes=atts))

    atts=Bunch(standard_name="northward_wind", long_name="V (N/S) wind speed",units="m s**-1")
    extra_vars.append(Bunch(name="v",data=cmip["v"],dims=dims,dtype="f",attributes=atts))

    atts=Bunch(standard_name="air_potential_temperature", long_name="Potential Temperature",units="kg kg**-1")
    extra_vars.append(Bunch(name="theta",data=cmip["t"],dims=dims,dtype="f",attributes=atts))

    atts=Bunch(standard_name="air_pressure", long_name="Pressure",units="Pa")
    extra_vars.append(Bunch(name="p",data=cmip["p"],dims=dims,dtype="f",attributes=atts))

    atts=Bunch(standard_name="", long_name="Layer thicknesses",units="m")
    extra_vars.append(Bunch(name="dz",data=cmip["dz"].astype("f"),dims=dims,dtype="f",attributes=atts))

    atts=Bunch(standard_name="", long_name="Layer height",units="m")
    extra_vars.append(Bunch(name="z",data=cmip["z"].astype("f"),dims=dims,dtype="f",attributes=atts))

    atts=Bunch(standard_name="", long_name="Topographic Height",units="m")
    ...
    atts=Bunch(standard_name="", long_name="latitude",units="degrees")
    extra_vars.append(Bunch(name="latitude",data=info.lat_data,dims=dims_2d,dtype="f",attributes=atts))

    atts=Bunch(standard_name="", long_name="longitude",units="degrees")
    extra_vars.append(Bunch(name="longitude",data=info.lon_data,dims=dims_2d,dtype="f",attributes=atts))

    atts=Bunch(standard_name="", long_name="xland",units="")
